...
public:
	 ...
	 * @param[in] address 地址
	 */
	static Socket::ptr CreateTCP(amot::Address::ptr address);

	/**
	 * @brief 创建UDP Socket(满足地址类型)
	 * @param[in] address 地址
	 */
	static Socket::ptr CreateUDP(amot::Address::ptr address);
	 ...
	 * 
	 * @param level 选项所在协议层，如SOL_SOCKET、IPPROTO_IP、IPPROTO_TCP
	 * @param option 需要访问的选项名,即指定的控制方式
	 * @param result 指向返回值的缓冲区
	 * @param len 选项值的最大长度
	 */
...
protected:
	...
	/**
	 * @brief 初始化sock, 对客户端Socket类对象进行初始化
	 */
