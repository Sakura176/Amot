...
concept AwaiterImplRestriction =
    std::is_base_of<Awaiter<R>, AwaiterImpl>::value;
...
struct Promise {
    ...
    await_transform(Task<_ResultType, _Executor> &&task) {
        SPDLOG_TRACE("await_transform task");
    ...
    SleepAwaiter
    await_transform(std::chrono::duration<_Rep, _Period> &&duration) {
    ...
    AwaiterImpl await_transform(AwaiterImpl awaiter) {
        SPDLOG_TRACE("await_transform AwaiterImpl!!!");
        awaiter.install_executor(&executor);
        return awaiter;
    }

    void return_value(ResultType val) {
        SPDLOG_TRACE("Promise return_value var {}", val);
